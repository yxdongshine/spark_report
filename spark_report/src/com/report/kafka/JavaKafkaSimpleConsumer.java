...
import com.report.threadpool.ReportResultTask;
import com.report.threadpool.ThreadPoolInstance;
import com.report.util.TaskUtil;
public class JavaKafkaSimpleConsumer {
     ...
     * @throws Exception
     */
    public synchronized void run( KafkaTopicPartitionInfo topicPartitionInfo,
        ...
        try {
			...
			synchronized (readOffSet) {
				while (true) {
					readOffSet = this.getLastOffSet(consumer, groupId, topic,
							partitionID, whichTime, clientName);
					if (readOffSet == -1) {
						// 当返回为-1的时候，表示异常信息
						if (times > this.maxRetryTimes) {
							throw new RuntimeException(
									"Fetch the last offset of those group:"
											+ groupId + " occur exception");
						}
						// 先休眠，再重新构建Consumer连接
						times++;
						this.sleep();
						consumer = this.createNewSimpleConsumer(consumer,
								topic, partitionID);
						continue;
					}
					// 正常情况下，结束循环
				...
				while (true) {
					// 构建获取数据的请求对象，
					// 给定获取数据对应的topic、partition、offset以及每次获取数据最多获取条数
					FetchRequest request = new FetchRequestBuilder()
							.clientId(clientName)
							.addFetch(topic, partitionID, readOffSet, 100000)//100000是单次读取消息的最大字节大小
							.build();
					...
					if (response.hasError()) {
						...
						System.out
								.println("Error fetching data from the Broker:"
										+ leaderBrokerHost + " Reason:" + code);
						if (numErrors > 5)
							break;
						if (code == ErrorMapping.OffsetOutOfRangeCode()) {
							// 异常表示是offset异常，重新获取偏移量即可
							readOffSet = this.getLastOffSet(consumer, groupId,
									topic, partitionID,
					...
					long numRead = 0;
					for (MessageAndOffset messageAndOffset : response
							.messageSet(topic, partitionID)) {
						// 校验偏移量
						long currentOffset = messageAndOffset.offset();
						if (currentOffset < readOffSet) {
							System.out.println("Found and old offset:"
									+ currentOffset + " Expection:"
									+ readOffSet);
							continue;
						}
						...
						//异步处理任务
						ReportResultTask rrTask = new ReportResultTask(messageKey, mv);
						ThreadPoolInstance.getThreadPoolInstancle().getThreadPoolExecutor().execute(rrTask);
						String info = currentOffset + "-->" + messageKey+ ": " + mv;
						System.out.println(info);
						TaskUtil.log.info("接受消息："+info);
						numRead++;
					}
					// 更新偏移量
